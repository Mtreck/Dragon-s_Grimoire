...
function renderItems() {
    ...
    if (items.length > 0) {
        ...
        itemTitle.classList.add('block', 'mb-2', 'text-lg', 'font-semibold'); // Tailwind classes for styling
    ...
    items.forEach((item, index) => {
        ...
        li.innerHTML = `${item.item} (x${item.quantity}, ${item.category}) <button onclick="editItem(${index})"> Editar</button> <button onclick="deleteItem(${index})"> Deletar</button>`;
...
function renderSpells() {
    ...
    if (spells.length > 0) {
        ...
        spellTitle.classList.add('block', 'mb-2', 'text-lg', 'font-semibold'); // Tailwind classes for styling
    ...
    spells.forEach((spell, index) => {
        ...
        li.innerHTML = `
            ...
            <div>
                <button onclick="editSpell(${index})"> Editar</button> 
                <button onclick="deleteSpell(${index})"> Deletar</button>
            </div>
            `;
        li.classList.add('bordered-list');
...
function renderNotes() {
    ...
    if (notes.length > 0) {
        ...
        notesTitle.classList.add('block', 'mb-2', 'text-lg', 'font-semibold'); // Tailwind classes for styling
...
function renderSubtopics(subtopics) {
    ...
    subtopics.forEach((subtopic, index) => {
        ...
        tr.innerHTML = `
            ...
            <td class="text-right"><button onclick="deleteSubtopic(${index})">Delete</button></td>
        `;
...
function closeModal(modalId) {
    ...
    document.body.style.overflow = ''; // Desbloquear rolagem de fundo
}
